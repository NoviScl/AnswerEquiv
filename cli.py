...
import numpy as np
import torch
from graph_nq import build_graph, build_rerank, build_path_data
...
def main():
    ...
    ## Basic parameters
    parser.add_argument("--task", default="qa", choices=["dpr", "qa", "qg", "bm25"],
                        type=str)
    parser.add_argument("--train_file", default="nqopen/nqopen-train.json",
                        type=str)
    parser.add_argument("--predict_file", default="nqopen/nqopen-dev.json",
                        type=str)
    ...
    parser.add_argument("--nq", action='store_true',
                        help="[For NQ] specify if you are experimenting with AmbigQA")
    ...
    parser.add_argument('--build_graph', action='store_true',
                        help="Build graph for transformer-XH")
    parser.add_argument('--build_rerank', action='store_true',
                        help="Build rerank data for BERT reranker")
    parser.add_argument('--build_paths', action='store_true',
                        help="Build paths data for training RNN reranker")
    parser.add_argument('--save_retrieve', action='store_true',
                        help="save retriever predictions or not")
    parser.add_argument('--do_bm25', action='store_true',
                        help="Add BM25 for retrieval")
    ...
    if args.build_graph:
        build_graph(args, logger)
    elif args.build_rerank:
        build_rerank(args, logger)
    elif args.build_paths:
        build_path_data(args, logger)
    else:
        run(args, logger)
