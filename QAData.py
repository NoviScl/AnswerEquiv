...
class QAData(object):
    ...
    def __init__(self, logger, args, data_path, is_training, passages=None):
        ...
        # self.data = self.data[ : 69205]
        logger.info("Number of examples: {}".format(len(self.data)))

        assert type(self.data)==list


        ## ignored this for TriviaQA and SQuAD
        # if not args.ambigqa:
    ...
    def load_tokenized_data(self, tokenizer):
        ...
        if self.load and os.path.exists(preprocessed_path):
            self.logger.info("Loading pre-tokenized QA data from {}".format(preprocessed_path))
    ...
    def load_dpr_data(self):
        dpr_retrieval_path = "{}_predictions.json".format(
            self.data_type+"_20200201" if self.args.wiki_2020 else self.data_type)
        dpr_retrieval_path = os.path.join(self.args.output_dir, dpr_retrieval_path)
    ...
    def load_dpr_data_bert(self, dpr_retrieval_path, dpr_tokenized_path):
        print ("DPR retrieval path: ", dpr_retrieval_path)
        # print ("DPR tokenized path: ", dpr_tokenized_path)
        ## hard-coding for inference on NQ train
        # dpr_tokenized_path = dpr_tokenized_path.replace('train_for_inference', 'train')
        print ("DPR tokenized path: ", dpr_tokenized_path)
        if os.path.exists(dpr_tokenized_path):
            self.logger.info("Loading DPR data from {}".format(dpr_tokenized_path))
            with open(dpr_tokenized_path, "r") as f:
                ...
                return
        self.logger.info("Start processing DPR data")
        # print (dpr_retrieval_path.replace("train", "train_for_inference"))
        if 'train' in dpr_retrieval_path and 'train_for_inference' not in dpr_retrieval_path:
            with open(dpr_retrieval_path.replace("train", "train_for_inference"), "r") as f:
                dpr_passages = json.load(f)
        else:
            with open(dpr_retrieval_path, "r") as f:
                dpr_passages = json.load(f)
        # print (len(dpr_passages))
        if self.args.ambigqa:
            ## added to convert original DPR data to AmbigQA DPR data
            # print ([d["orig_idx"] for d in self.data])
            ## clsi: no need this step as we did our DPR retrieval on the AmbigQA dev set, not on NQ-open
            # dpr_passages = [dpr_passages[d["orig_idx"]] for d in self.data]
            pass
        elif self.is_training and self.args.nq:
        ...
        for i, (q_input_ids, q_attention_mask, retrieved) in \
            ...
            for _p_input_ids in p_input_ids:
                ...
                a_input_ids = [list(a) for a in set(tuple(a) for a in a_input_ids)] ## deduplicate
                for _a_input_ids in a_input_ids:
                    if _a_input_ids is None or len(_a_input_ids) == 0:
                        continue 
                    ...
                    for j in range(len(_p_input_ids)-len(_a_input_ids)+1):
                        if _p_input_ids[j:j+len(_a_input_ids)]==_a_input_ids:
                            ...
                            if decoded_a_input_ids==self.decode(_p_input_ids[j:k]):
                                ...
                                all_detected += 1
            # if (self.args.ambigqa or not self.args.nq) and self.is_training:
            #     ## positives: passage ids with detected spans
            ...
            #     positives = [j for j in _positives if normalize_answer(self.decode(p_input_ids[j][:]))==gold_title]
            #     positive_contains_gold_title.append(len(positives)>0)
            #     if len(positives)==0:
            #         positives = _positives[:20]
            #     negatives = [j for j, spans in enumerate(detected_spans) if len(spans)==0][:50]
            if self.is_training:
                ## keeping too many positives and negatives causes large RAM consumption
                positives = [j for j, spans in enumerate(detected_spans) if len(spans)>0][ : 10]
                negatives = [j for j, spans in enumerate(detected_spans) if len(spans)==0][ : 24]
                if len(positives)==0:
                    continue
            else:
        ...
        print ("all detected spans: ", all_detected)
    ...
    def evaluate(self, predictions, n_paragraphs=None):
        ...
        with open('/data3/private/clsi/AmbigQA/data/triviaqa_alias/test.json', 'r') as f:
        ...
        if n_paragraphs is None:
            ...
            for (prediction, dp) in zip(predictions, self.data):
                ...
                # print (prediction, dp["answer"])
                ems.append(get_exact_match(prediction, dp["answer"]))
                if get_exact_match(prediction, dp["answer"]) < get_exact_match(prediction, multi_data[i]["answer"]):
                    print (prediction, dp["answer"], multi_data[i]["answer"], multi_data[i]['question'])
                i += 1
            return ems
    ...
    def save_predictions(self, predictions):
        assert len(predictions)==len(self), (len(predictions), len(self))
        save_path = os.path.join(self.args.output_dir, "{}{}_eval_predictions.json".format(
...
class AmbigQAData(QAData):
    def __init__(self, logger, args, data_path, is_training, passages=None):
        ...
        for i, d in enumerate(self.data):
            ...
            #     print (answers)
            # answers = list(set([_a for answer in answers for _answer in answer for _a in _answer]))
            ...
            self.data[i]["orig_idx"] = id_to_orig_idx[d["id"]] ## orig_idx: id in NQ-Open
    ...
    def load_dpr_data(self):
        ...
        # print ("dpr_retrieval_path: {}".format(dpr_retrieval_path))
        # print ("dpr_tokenized_path: {}".format(dpr_tokenized_path))
        return self.load_dpr_data_bert(dpr_retrieval_path, dpr_tokenized_path)
    ...
    # override
    def evaluate_thres(self, predictions, n_paragraphs=None, thres=0.05):
        assert len(predictions)==len(self), (len(predictions), len(self))
        f1s, f1s_wo_dupli = [], []
            ...
            for (prediction, dp) in zip(predictions, self.data):
                ...
                for p in prediction:
                    if normalize_answer(p["text"]) not in preds:
                        if p["log_softmax"]>np.log(thres) or len(preds)==0:
                            preds.append(normalize_answer(p["text"]))
                        if p["log_softmax"]<=np.log(thres) or len(preds)==3:
                            break
                ...
                # print ([p['passage_index'] for p in preds])
                f1s.append(np.max([get_f1(answer, preds) for answer in dp["answer"]]))
        self.logger.info("threshold: {}, Performance: {}".format(thres, np.mean(f1s)))
        return f1s
    
    def evaluate(self, predictions, n_paragraphs=None):
        for t in range(35, 70, 5):
            t = t * 10.0 / 1000.0
            f1s = self.evaluate_thres(predictions, n_paragraphs=n_paragraphs, thres=t)
        return f1s
